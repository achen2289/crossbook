...
Polyscope consumes only the **public gateway** (`/v1/events`, `/v1/markets`, `/v1/markets/{slug}/bbo` + `/book`, `/v2/tags`, `/v1/search`) for its scanner, compare, and movers views, plus the **retail account endpoints read-only** (`/v1/portfolio/positions`, `/v1/portfolio/activities`, `/v1/account/balances`) for the portfolio panel. It places no orders and calls no mutating endpoint. Because the retail REST API has no price history (§7.1), the movers view is powered by Polyscope's own periodic sampler over the public BBO endpoints, rate-limited well under the 20 rps budget.
